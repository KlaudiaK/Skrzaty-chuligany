    ...
    while (stan != InFinish) {
        ...
        switch ( status.MPI_TAG ) {
            ...
            case RELEASE_GUN:
                ...
                int count_gp = 0;
                while (gpReqQueueHead != NULL && count_gp < nGunpoint && nGunpoint > 0) {
            ...
            case GUN_PRODUCED:
                ...
                pthread_mutex_lock(&mutex);
                nGun++;
                printf("Przed usunięciem Gun_PRODUCED\n");
                ...
                debug("Przed while odbieranie GUN_PRODUCED od %d, zwiekszam dostepnosc broni %d", status.MPI_SOURCE, nGun);

                printList(gunReqQueueHead);
                while (gunReqQueueHead != NULL && count < nGun && nGun > 0) {
                    if (gunReqQueueHead->id == rank)
                    {
                        debug("AckCount %d ", ackCountGun );
                        if (ackCountGun == size - 1
                        && isElementAmongFirst(gunRequestQueue, rank, nGun) == 1
                        ){
                            ...
                            pthread_cond_signal(&condition);
                             ...
                             //pthread_cond_wait(&condition, &mutex);
                               gunReqQueueHead = gunReqQueueHead->next; // Taki eksperyment, bo tu też się kleszczy 
                        }
                    } else {
                         debug("AckCount %d bloccccccc");
                ...
                debug("Zakonczyyyyyyylem odbieranie GUN_PRODUCED od %d, zwiekszam dostepnosc broni", status.MPI_SOURCE);
                sort(&gunRequestQueue);
                pthread_mutex_unlock(&mutex);
                break;
            default:
